...
FUEL_CONSUMPTION = 258.4  # 燃油消耗率 [g/(kW·h)]，≤258.4
...
# 排量使用给定值0.817 L，而非计算值
DISPLACEMENT = 0.817  # 排量 [L]（给定值）
MEAN_PISTON_SPEED = 2 * STROKE * RATED_SPEED / 60  # 活塞平均速度 [m/s]
# 平均有效压力使用给定值650 kPa
MEAN_EFFECTIVE_PRESSURE = 650  # 平均有效压力 [kPa]（给定值）
...
# 旋转质量重心半径 [m]
# 注：以下数值来自SD195柴油机结构参数（附录3）
# 实际设计中应根据零件图纸的质心计算结果确定
CRANK_PIN_COG_R = 57.5e-3      # 曲柄销重心半径，等于曲柄半径（质心在轴心）
CRANK_WEB_COG_R = 41.5e-3      # 曲柄臂重心半径（质心在曲柄臂几何中心）
BALANCE_WEIGHT_COG_R = 68.8e-3 # 平衡重重心半径（质心在平衡重几何中心）
...
# 活塞面积（用于压力单位转换）[m²]
PISTON_AREA_CM2 = PISTON_AREA * 1e4  # 转换为cm²便于计算
